{{-- resources/views/reports/pdf/sale_return.blade.php --}}
@extends('reports.pdf.layout')

@section('content')

@php
  $total    = $data->sum('total_refund');
...
<table>
  <thead>
    <tr>
      ...
      <th class="right">Total Refund (Rp)</th>
    </tr>
  </thead>
  <tbody>
    ...
    <tr>
      ...
      <td class="mono">{{ $row->sale?->invoice_number ?? '-' }}</td>
      <td>{{ $row->sale?->customer?->name ?? 'Walk-in' }}</td>
      <td>{{ $row->user?->name }}</td>
      <td>{{ $row->refund_method ?? '-' }}</td>
      <td><span class="badge {{ $statusColor }}">{{ strtoupper($row->status) }}</span></td>
      <td class="center">{{ $row->items->count() }}</td>
      <td class="right">{{ number_format($row->total_refund, 0, ',', '.') }}</td>
    </tr>
    @empty
  ...
  <tfoot>
    <tr>
      <td colspan="9" class="right">TOTAL REFUND</td>
