...
.pos-header-logo svg { width: 17px; height: 17px; color: #fff; }
.pos-header-store {
    flex: 1; min-width: 0;
...
.pos-clock-date { font-size: 11px; color: var(--text-muted); }
...
@media (min-width: 640px) { .pos-cashier-name { display: block; } }
...
@media (min-width: 768px) { .products-scroll { padding-bottom: 12px; } }
...
.product-card.in-cart { border-color: var(--blue); }
...
.product-card.in-cart .product-badge { display: flex; }
...
/* Empty & Loading */
.products-empty, .products-loading {
    display: flex; flex-direction: column;
...
.products-empty p { font-size: 14px; color: var(--text-muted); }
.spinner {
...
@media (min-width: 768px)  { .cart-sidebar { display: flex; width: 280px; } }
@media (min-width: 1024px) { .cart-sidebar { width: 320px; } }
@media (min-width: 1280px) { .cart-sidebar { width: 360px; } }

/* ── FAB Cart Button (Mobile) ─────────────────────────────────── */
.fab-cart {
    display: none; /* shown by JS when cart has items on mobile */
    position: fixed; bottom: calc(20px + env(safe-area-inset-bottom, 0px)); right: 16px;
    z-index: 40;
    background: var(--blue);
...
.fab-cart:active { transform: scale(.96); }
.fab-badge {
    background: rgba(255,255,255,.25);
    font-size: 11px; font-family: 'DM Mono', monospace;
    padding: 1px 7px; border-radius: 99px;
}
.fab-total { font-family: 'DM Mono', monospace; font-size: 13px; }
...
.sheet-handle { width: 36px; height: 4px; background: var(--border); border-radius: 99px; }
...
.qty-val {
    ...
    min-width: 20px; text-align: center; color: var(--text);
}
...
.sheet-footer { flex-shrink: 0; padding: 16px; border-top: 1px solid var(--border); }

/* Summary (shared) */
.s-summary { display: flex; flex-direction: column; gap: 5px; margin-bottom: 14px; }
.s-row {
...
.s-total-val { color: var(--blue); font-family: 'DM Mono', monospace; }

/* Payment section (shared) */
.p-section { display: flex; flex-direction: column; gap: 10px; }
.cash-row-m { display: flex; flex-direction: column; gap: 5px; }
.cash-label-m { font-size: 12px; font-weight: 500; color: var(--text-muted); }
...
.cash-inp:focus { border-color: var(--blue); background: var(--white); }

.change-row-m {
    display: flex; justify-content: space-between; align-items: center;
    background: var(--green-bg); border-radius: var(--radius-xs);
    padding: 10px 14px; font-size: 14px; font-weight: 600; color: var(--green-text);
}
.change-mono { font-family: 'DM Mono', monospace; }

.method-g { display: grid; grid-template-columns: repeat(3, 1fr); gap: 7px; }
.m-btn {
...
.m-btn:hover { border-color: var(--blue-mid); color: var(--blue); }
.m-btn.active { border-color: var(--blue); background: var(--blue-light); color: var(--blue); }
...
.btn-pay-m:hover { background: var(--blue-dark); }
.btn-pay-m:active { transform: scale(.98); }
.btn-pay-m:disabled { opacity: .5; cursor: not-allowed; transform: none; }
...
@media (min-width: 600px) {
    .modal { border-radius: 20px; padding-bottom: 24px; animation: fadeUp .2s ease both; }
}
...
.receipt-content { padding: 16px 20px; }
.receipt-store { text-align: center; margin-bottom: 16px; }
.receipt-store-name { font-size: 16px; font-weight: 700; color: var(--text); }
...
.receipt-item-name { flex: 1; padding-right: 8px; }
.receipt-item-amt { font-family: 'DM Mono', monospace; white-space: nowrap; }
...
.receipt-row.change { color: var(--green); font-weight: 600; }
.receipt-mono { font-family: 'DM Mono', monospace; }
.receipt-thanks {
...
@media print {
    ...
    .modal-bg { display: block !important; background: none; position: static; }
    .modal { box-shadow: none; border-radius: 0; max-height: none; }
...
<div id="cart-sheet" class="sheet">
    ...
    <div class="sheet-footer">
        ...
        <div id="sheet-pay-section" class="p-section hidden">
            <div class="cash-row-m">
                ...
                <div class="cash-wrap">
                    ...
                    <input type="number" id="paid-input-mobile" placeholder="0"
                        class="cash-inp" oninput="updateChange('mobile')">
                </div>
            </div>
        ...
        <div id="sheet-empty-cta" style="text-align:center; padding: 8px 0; font-size: 13px; color: var(--text-faint);">
...
async function processPayment() {
    ...
    if (state.paymentMethod === 'cash' && paid < total) {
        alert('Uang diterima kurang dari total!'); return;
    }

    const payBtns = ['pay-btn', 'pay-btn-mobile'].map(id => document.getElementById(id)).filter(Boolean);
    payBtns.forEach(b => { b.disabled = true; b.textContent = 'Memproses…'; });

    try {
        const cr = await fetch('{{ route('pos.api.sale.create') }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, Accept: 'application/json' },
            body: JSON.stringify({ items: state.cart.map(c => ({ product_id: c.product.id, qty: c.qty })) }),
        });
        ...
        const pr = await fetch(`/pos/api/sale/${cData.sale_id}/pay`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, Accept: 'application/json' },
            body: JSON.stringify({ payment_method: state.paymentMethod, paid: state.paymentMethod === 'cash' ? paid : total }),
        });
...
document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Escape') { closeReceipt(); closeSheet(); }
});
