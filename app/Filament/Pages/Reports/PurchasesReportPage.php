...
class PurchasesReportPage extends Page implements HasTable
{
    ...
    const REPORT_TYPE = 'purchases';
    const REPORT_TITLE = 'Laporan Pembelian';
    ...
    {
        return 'Rp ' . number_format($this->getFilteredQuery()->sum('total'), 0, ',', '.');
    }
    ...
    public function getAverageTransaction(): string
    {
        ...
        if ($count === 0)
            return 'Rp 0';
        return 'Rp ' . number_format($this->getFilteredQuery()->avg('total'), 0, ',', '.');
    }
    public function getTotalUnpaid(): string
    {
        return 'Rp ' . number_format($this->getFilteredQuery()->sum('due'), 0, ',', '.');
    }
    public function getTotalPaid(): string
    {
        return 'Rp ' . number_format($this->getFilteredQuery()->sum('paid'), 0, ',', '.');
    }
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('reference_number')->label('No. Referensi')->searchable()->fontFamily('mono')->copyable(),
                TextColumn::make('supplier_invoice')->label('No. Invoice Supplier')->searchable()->placeholder('-'),
                TextColumn::make('supplier.name')->label('Supplier')->placeholder('Walk-in'),
                ...
                TextColumn::make('status')->label('Status PO')->badge()
                    ->formatStateUsing(fn($s) => match ($s) {
                        ...
                        'partial' => '⏳ Partial', 'cancelled' => '❌ Cancelled', default => $s,
                    })
                    ->color(fn($s) => match ($s) {
                        'received' => 'success', 'ordered' => 'info', 'partial' => 'warning', 'cancelled' => 'danger', default => 'gray',
                    }),
                TextColumn::make('payment_status')->label('Status Bayar')->badge()
                    ->formatStateUsing(fn($s) => match ($s) {
                        'paid' => '✅ Lunas', 'partial' => '⏳ Sebagian', 'unpaid' => '❌ Belum Bayar', default => $s,
                    })
                    ->color(fn($s) => match ($s) {
                ...
                TextColumn::make('paid')->label('Terbayar')->money('IDR')->sortable()
                    ->summarize(Sum::make()->money('IDR')->label('Total Terbayar')),
                TextColumn::make('due')->label('Sisa Hutang')->money('IDR')->sortable()
            ...
            ->filters([
                ...
                SelectFilter::make('status')->label('Status PO')
                    ->options(['draft' => 'Draft', 'ordered' => 'Ordered', 'received' => 'Received', 'partial' => 'Partial']),
                SelectFilter::make('payment_status')->label('Status Bayar')
                    ->options(['paid' => 'Lunas', 'partial' => 'Sebagian', 'unpaid' => 'Belum Bayar']),
